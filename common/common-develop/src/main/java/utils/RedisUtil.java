...
public class RedisUtil {
    ...
    public String getString(String key) {
        ...
        if(key == null){
            ...
            rel= stringRedisTemplate.opsForValue().get(key);
            log.info("getString:{} rel:{}",key,rel);
        }
        return rel;
    }
...
//        redisTemplate.opsForList().leftPushAll(key,"w","x","y");
    ...
    public Long getIncrementByKey(String key,long timeout){
        RedisAtomicLong entityIdCounter = new RedisAtomicLong(key, redisTemplate.getConnectionFactory());
        Long increment = entityIdCounter.incrementAndGet();
    ...
    public void delIncrementKey(String key){
        RedisAtomicLong entityIdCounter = new RedisAtomicLong(key, redisTemplate.getConnectionFactory());
        entityIdCounter.expire(1,TimeUnit.MILLISECONDS);
    }

    /**
     * @Description: 转string存储redis
     * @Param: [value]
     * @Return: java.lang.String
     * @Author: zhanglianhai
     * @Date: 2020/12/28
     ...
     * @Date: 2022/02/24
     **/
    public void setHashValue(String key,Map map) {
        redisTemplate.opsForHash().putAll(key,map);
    }
