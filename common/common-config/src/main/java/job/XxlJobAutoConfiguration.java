...
import job.properties.XxlExecutorProperties;
import job.properties.XxlJobProperties;
...
public class XxlJobAutoConfiguration {
    ...
    public XxlJobSpringExecutor xxlJobSpringExecutor(XxlJobProperties xxlJobProperties, Environment environment,
        ...
        log.info(">>>>>>>>>>> xxl-job configuration：{}", JSONUtil.toJsonStr(xxlJobProperties));
        log.info(">>>>>>>>>>> xxl-job application name：{}",appName);
        xxlJobSpringExecutor.setAppname(appName);
        xxlJobSpringExecutor.setAddress(executor.getAddress());
        xxlJobSpringExecutor.setIp(executor.getIp());
        xxlJobSpringExecutor.setPort(executor.getPort());
        if (StringUtils.isEmpty(executor.getAccessToken())){
            xxlJobSpringExecutor.setAccessToken(EnvAccountTokenEnum.valAccountToken(environment.getActiveProfiles()[0]));
        }else {
        ...
        if (!StringUtils.hasText(xxlJobProperties.getAdmin().getAddresses())) {
            ...
            log.info(">>>>>>>>>>> xxl-job dispatch center：{}",serverList);
